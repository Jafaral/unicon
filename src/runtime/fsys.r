...
function{0,1} open(fname, spec)
    ...
    *   - crypto material handle + mode "e": derive an operation (UTR 24)
    *   - SSH session/channel: open a channel or SFTP on the same session
    */
         ...
         if (BlkD(fname,File)->status & Fs_Crypto) {
            ...
            for (a = 0; a < n; a++) {
               ...
               if ((rc = crypto_setattr(cf, nbuf, v, vlen)) != 0) {
                  crypto_free(cf); set_errortext(rc); fail;
                  }
               }
            ...
            if (src == NULL) { crypto_free(cf); set_errortext(1312); fail; }
            ...
            for (a = 0; a < n; a++) {
               ...
               if (!(BlkD(attr[a],File)->status & Fs_Crypto)) {
                  crypto_free(cf); set_errortext(1312); fail;
                  }
      ...
      if (status & Fs_Crypto) {
         ...
         if (crypto_op == CRYPTO_OP_HASH) {
            ...
            if (!cnv:C_string(fname, alg)) {
               crypto_free(cf); runerr(103, fname);
               }
            cf->md = EVP_get_digestbyname(alg);
            if (cf->md == NULL) {
               crypto_free(cf); set_errortext(1311); fail;
               }
            for (a = 0; a < n; a++) {
               tended struct descrip as;
               char nbuf[64];
            ...
            if (!have_op) {
               ...
               if (cf->op == CRYPTO_OP_MATERIAL) {
                  crypto_free(cf); set_errortext(1316); fail;
                  }

               {
                  struct CryptoFile *m;
                  m = crypto_open_material(&fname, crypto_raw, attr, n, &rc);
                  if (m == NULL) { crypto_free(cf); set_errortext(rc); fail; }
                  rc = crypto_borrow(cf, m);
                  crypto_free(m);
                  if (rc != 0) { crypto_free(cf); set_errortext(rc); fail; }
                  }
