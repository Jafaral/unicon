...
\texttt{conn["cipher"]}); an unknown name is a runtime error and an
unpopulated field fails.
\texttt{f["groups"]}, \texttt{s["authmethods"]}, \texttt{h["san"]}, and
\texttt{conn["certchain"]} produce a list (a single item is still a
list of length 1); other peeks are a string or integer.
\texttt{key(f)} generates the populated field names, matching
...
non-destructive hash \texttt{digest} peek uses \texttt{h["\textit{name}"]}.
...
\texttt{key(f)} generates populated status-field names on a cryptographic
