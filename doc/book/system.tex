...
\texttt{key(f)} generates populated field names.
...
\texttt{key(c)} generates populated field names.
...
\texttt{h["expired"]} introspect a certificate. On a hash handle, \texttt{h["digest"]} peeks
the current digest without finalizing. On encrypt/decrypt,
\texttt{h["iv"]} peeks the IV that was set or generated.
\texttt{key(h)} generates populated field names.
