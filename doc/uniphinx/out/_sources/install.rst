...
- A TeX engine on your ``PATH``: ``pdflatex``, ``xelatex``, or
  ``lualatex``. uniphinx picks the first one it finds.
...
Compact (recommended for uniphinx):
...
What uniphinx LaTeX output needs
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
accept the boxed placeholder uniphinx emits for ``.svg`` paths.
...
That compiles the ``uniphinx`` package, links ``./uniphinx``, and
installs a copy to ``../../bin/uniphinx``.
